...
>
> $\\tag{M1} \\label{M1}$ (Non-negativity) $d(x,y) \\geq 0$, and $d(x,y) = 0 \\iff x = y.$
> 
> $\\tag{M2} \\label{M2}$ (Symmetric) $d(x,y) = d(y,x).$
> 
> $\\tag{M3} \\label{M3}$ (Triangle Inequality) $d(x,z) \\leq d(x,y) + d(y,z).$
...
> 
> After cancelling, we just need to show $a_{1}b_{1} + a_{2}b_{2} \\leq \\sqrt{(a_{1}^2 + a_{2}^2)(b_{1}^2 + b_{2}^2)}.$ $\\tag{1} \\label{1}$
> 
...
>  $a_{1}b_{1} + a_{2}b_{2} \\leq \\sqrt{(a_{1}^2 + a_{2}^2)(b_{1}^2 + b_{2}^2)} \\implies d(x,z) \\leq d(x,y) + d(y,z)$ by $\\ref{1}$.
> 
